...
class TestRAGActionsP1OpenAI:
    ...
    def test_vector_store_rejects_mismatched_dimensions(self, in_memory_store):
        ...
        with pytest.raises(ValueError, match="dimension mismatch"):
            in_memory_store.add(
                ids=['doc2'],
                texts=['Doc 2'],
                embeddings=[[0.1, 0.2]]  # Only 2 dims
            )
    ...
    def test_action_registration_namespaces(self, engine):
        ...
        assert engine.actions_registry['embedding.create'] is engine.actions_registry['actions.embedding_create']
        assert engine.actions_registry['vector.store'] is engine.actions_registry['actions.vector_store']
        assert engine.actions_registry['vector.query'] is engine.actions_registry['actions.vector_query']

    def test_vector_store_with_checkpoint(self, engine, in_memory_store, sample_embeddings):
        ...
        new_engine.restore_rag_state = lambda s: new_store.restore_state(s.get('vector_store', {}))
...
class TestRAGActionsP2:
    ...
    def test_vector_store_auto_embed(self, engine, in_memory_store):
        ...
        with patch('openai.OpenAI') as mock_openai_class:
            ...
            result = engine.actions_registry['vector.store'](
                state={},
                texts="Auto-embed this text"
            )

            assert result['stored'] == 1
            mock_client.embeddings.create.assert_called_once()

    def test_vector_store_handles_duplicate_ids(self, in_memory_store, sample_embeddings):
...
class TestRAGActionsIntegration:
    ...
    def test_embedding_with_llm_call(self, engine, in_memory_store):
        ...
        with patch('openai.OpenAI') as mock_openai_class:
            ...
            mock_chat_response.choices[0].message.content = "Based on the context..."
            mock_chat_response.usage = Mock()
            mock_chat_response.usage.model_dump = Mock(return_value={})
            ...
            engine.actions_registry['vector.store'](
                state={},
                texts=["Important fact: The capital of France is Paris."]
            )
