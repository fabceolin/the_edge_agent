...
def _stamp(fd: int, owner: str) -> None:
    """Write who holds the lock — diagnostics only; flock is the real thing."""
    os.ftruncate(fd, 0)
    os.pwrite(fd, f"{os.getpid()} {owner or 'unnamed'} {int(time.time())}\n".encode(), 0)
...
def owns(repo: str | Path, owner: str) -> bool:
    """Whether this process holds the lock under that owner token."""
    return is_held(repo) and owner in read_holder(repo)


def refresh_stamp(repo: str | Path, owner: str) -> bool:
    """Rewrite the diagnostic stamp, if we are the ones holding the lock.

    Under flock there is nothing to keep alive — the hold lasts exactly as long
    as the process does.  This only refreshes what a waiter prints when it
    reports who it is waiting on, and doubles as an ownership assertion for
    callers that used to depend on a heartbeat.
    """
    held = _HELD.get(_key(repo))
    if not held or owner not in read_holder(repo):
        return False
    _stamp(held[1], owner)
    return True
