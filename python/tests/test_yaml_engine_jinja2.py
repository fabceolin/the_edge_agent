...
class TestContextVariables:
    ...
    def test_yaml_001_int_003_state_accessible(self, engine):
        ...
        result = engine._process_template(
            "Key={{ state.key }}, Nested={{ state.nested.a }}", state
        )
...
class TestRunBlockCompatibility:
    ...
    def test_yaml_001_int_008_run_executes_python(self, engine):
        ...
        config = {
            "nodes": [{"name": "compute", "run": 'result = {"sum": 1 + 2 + 3}'}],
            "edges": [
...
class TestStrictUndefined:
    ...
    def test_yaml_001_int_010_default_filter_with_none(self, engine):
        """YAML-001-INT-010: | default(val) works with None values.

        Note: DotDict returns None for missing keys (not Undefined), so we need
        to use default(value, true) to also replace None values.
        """
        # With default(val, true), None is also replaced with the default
        result = engine._process_template(
            '{{ state.missing | default("fallback", true) }}', {}
        )
        assert result == "fallback"

        # Standard default only replaces Undefined, not None
        result_none = engine._process_template(
            '{{ state.missing | default("fallback") }}', {}
        )
        assert result_none is None  # DotDict returns None for missing keys
...
class TestCaching:
    ...
    def test_yaml_001_unit_017_template_cached(self, engine):
        ...
        assert (
            cache_key in engine._template_cache
            or f"tmpl:{template}" in engine._template_cache
        )
...
class TestWhenConditions:
    ...
    def test_yaml_001_when_jinja2_expression(self, engine):
        ...
        assert (
            events_a[-1].get("state", events_a[-1]).get("result", {}).get("path") == "a"
        )

        # Test value <= 5 takes path_b
        events_b = list(graph.invoke({"value": 3}))
        assert (
            events_b[-1].get("state", events_b[-1]).get("result", {}).get("path") == "b"
        )
...
""",
        ...
        final = events[-1]
        assert "Hello, World!" in final.get("state", final).get("result", {}).get(
            "message", ""
        )
