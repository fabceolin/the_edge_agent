...
def mock_streaming_chunks():
    ...
    final_chunk.usage.model_dump = Mock(
        return_value={"prompt_tokens": 10, "completion_tokens": 4, "total_tokens": 14}
    )
...
class MockOpenAIClients:
    """
    Context manager that mocks both OpenAI and AzureOpenAI clients.

    This is necessary because llm_call auto-detects Azure when AZURE_OPENAI_*
    environment variables are set, so we need to mock both client classes.

    Usage:
        with MockOpenAIClients() as mock_class:
            mock_client = Mock()
    ...
    def __enter__(self):
        ...
        self._azure_patcher = patch("openai.AzureOpenAI")
        self.mock_openai = self._openai_patcher.start()
        self.mock_azure = self._azure_patcher.start()
        return self

    def __exit__(self, *args):
        self._openai_patcher.stop()
        self._azure_patcher.stop()

    @property
    def return_value(self):
        return self.mock_openai.return_value

    @return_value.setter
    def return_value(self, value):
        self.mock_openai.return_value = value
        self.mock_azure.return_value = value
...
class TestLLMEnhancedActionsP0:
    ...
    def test_existing_llm_call_unchanged(self, engine):
        ...
        assert (
            ...
            is engine.actions_registry["actions.llm_call"]
        )
    ...
    def test_llm_retry_respects_max_retries(self, engine):
        ...
        # Create mock client that always raises RateLimitError
        mock_client = Mock()
        mock_client.chat.completions.create.side_effect = rate_limit_error

        # Mock both OpenAI and AzureOpenAI (for Azure env detection) plus time.sleep
        with MockOpenAIClients() as mock_openai_class, patch(
            "the_edge_agent.actions.llm_actions.time.sleep"
        ):
    ...
    def test_llm_tools_invalid_action_error(self, engine):
        ...
        tools = [
            {"name": "test_tool", "description": "Test", "action": "nonexistent.action"}
        ]
    ...
    def test_llm_tools_action_injection_blocked(self, engine):
        ...
        for tool in malicious_tools:
            ...
            assert result.get("success") is False
            assert "Invalid action reference" in result.get(
                "error", ""
            ) or "not found" in result.get("error", "")
        ...
        with patch("the_edge_agent.yaml_engine.time.sleep"):
            ...
            with MockOpenAIClients() as mock_openai_class:
                ...
                def side_effect(*args, **kwargs):
                    ...
                    if call_count[0] < 2:
                        error = APIError(
                            message="Service unavailable", request=Mock(), body=None
                        )
...
class TestLLMEnhancedActionsP1:
    ...
    def test_llm_stream_yields_chunks(self, engine, mock_streaming_chunks):
        ...
        with MockOpenAIClients() as mock_openai_class:
            ...
            result = llm_stream(
                state={}, model="gpt-4", messages=[{"role": "user", "content": "test"}]
            )
    ...
    def test_llm_tools_dispatches_to_action(self, engine, mock_openai_response):
        ...
        with MockOpenAIClients() as mock_openai_class:
            ...
            tool_call_response.choices[0].message.tool_calls[
                0
            ].function.arguments = '{"query": "python tutorials"}'
    ...
    def test_dual_namespace_llm_stream(self, engine):
        ...
        assert (
            ...
            is engine.actions_registry["actions.llm_stream"]
        )
    ...
    def test_dual_namespace_llm_retry(self, engine):
        ...
        assert (
            engine.actions_registry["llm.retry"]
            is engine.actions_registry["actions.llm_retry"]
        )
    ...
    def test_dual_namespace_llm_tools(self, engine):
        ...
        assert (
            engine.actions_registry["llm.tools"]
            is engine.actions_registry["actions.llm_tools"]
        )
    ...
    def test_llm_tools_with_registered_actions(self, engine):
        ...
        with MockOpenAIClients() as mock_openai_class:
            ...
            tool_response.choices[0].message.tool_calls[
                0
            ].function.arguments = '{"query": "test query"}'
