...
class TestBuiltinActions:
    ...
    def test_unit_019_unknown_action_raises_valueerror(self, engine):
        ...
        config = {
            ...
            "nodes": [{"name": "test", "uses": "nonexistent.action", "with": {}}],
            "edges": [
                {"from": "__start__", "to": "test"},
                {"from": "test", "to": "__end__"},
            ],
    ...
    def test_unit_022_output_dict_none_to_empty_string(self, engine):
        """YAML-001-UNIT-022: output dict converts None template values to empty string.

        Regression test for: Error in node 'summary': argument of type 'NoneType' is not iterable
        When an action returns None for a field and output template maps it, downstream
        nodes using 'in' operator would fail. Fix converts None to "" in output processing.
        """

        # Register a mock action that returns None for content field
        def mock_action_with_none(state, **kwargs):
            return {"content": None, "status": "ok"}

        engine.actions_registry["test.none_action"] = mock_action_with_none

        config = {
            "nodes": [
                {
                    "name": "test",
                    "uses": "test.none_action",
                    "with": {},
                    "output": {
                ...
                },
                {
                    "name": "check",
                    "run": """
# This would fail with TypeError if result_content is None
content = state.get("result_content", "")
contains_marker = "MARKER" in content
return {"checked": True, "contains_marker": contains_marker}
""",
                },
            ],
            "edges": [
        ...
        # Key assertions:
        # 1. None value should be converted to empty string
        assert final_state["result_content"] == ""
        # 2. Non-None value should be preserved
        assert final_state["result_status"] == "ok"
        # 3. Downstream node should execute without TypeError
        assert final_state["checked"] is True
        assert final_state["contains_marker"] is False
    ...
    def test_unit_052_custom_action_registration(self, engine):
        ...
        config = {
            "nodes": [{"name": "test", "uses": "math.multiply", "with": {"factor": 3}}],
            "edges": [
                {"from": "__start__", "to": "test"},
                {"from": "test", "to": "__end__"},
            ],
...
class TestTemplateProcessing:
    ...
    def test_unit_041_state_template(self, engine):
        ...
        result = engine._process_template(
            "Hello {{ state.name }}, age {{ state.age }}", state
        )
...
class TestBuiltinActionsRegistry:
    ...
    def test_int_012_file_read_action(self, engine):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
            ...
            config = {
                "nodes": [
                    {"name": "read", "uses": "file.read", "with": {"path": f.name}}
                ],
                "edges": [
