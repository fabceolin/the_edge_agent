"""
Memory Backend Infrastructure for YAMLEngine (TEA-BUILTIN-001.1).

This module provides pluggable memory backends for key-value storage with
optional TTL (time-to-live) support. Memory backends enable agents to store
and retrieve conversation context, user preferences, and other stateful data.

The default InMemoryBackend uses thread-safe storage with monotonic time for
TTL tracking, ensuring consistent expiration behavior across sessions.
...
Example:
    ...
    >>> backend.store("preferences", {"theme": "dark"}, namespace="user_123")
    True
    >>>
    >>> # Retrieve values
    ...
    >>>
    >>> # Check existence
    >>> backend.exists("user_name")
    True
    >>>
    >>> # Namespace isolation
    >>> backend.retrieve("preferences", namespace="user_123")
    {'theme': 'dark'}
    >>>
